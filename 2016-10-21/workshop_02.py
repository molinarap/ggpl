...
# funzione che crea il telaio piano
def createLast(dist, pilar_height_tot, beam_weight_tot, i, y, arc_all):
	...
	if i == len(beams_weights) - 1:
		...
		if y != len(pilars_heights) - 1:
			y=y+1
			return createLast(dist, pilar_height_tot, 0, 0, y, arc_all)
		else:
			VIEW(arc_all)
	else:
		i=i+1
		arc_all = STRUCT([arc_all,p1_t,t1_t])
		return createLast(dist, pilar_height_tot, beam_weight_tot, i, y, arc_all)
